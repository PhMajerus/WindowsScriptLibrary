...
' characters allowed as part of a URI.
' 
' Note if you need to encode a string to pass as a querystring value, use
' EncodeURIComponent instead to encode the ;,/?:@&=+$# characters as well.
' 
' Dependencies:
...
Function EncodeURI (URI)
	' Unreserved marks, lists the characters that are allowed as it in a URI
	...
	While I <= L
		...
		Else
			...
			Else
				...
				B3 = "%" & Right("0" & Hex(B3), 2)
				B4 = "%" & Right("0" & Hex(B4), 2)
				EncodeURI = EncodeURI & (B1 & B2 & B3 & B4)
			End If
		End If
		' loop
		I = I + 1
